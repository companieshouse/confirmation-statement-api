...
dependency-check:
	...
	if [  -f "$${suppressions_path}" ]; then \
		...
		mvn org.owasp:dependency-check-maven:check -DfailBuildOnCVSS=$(dependency_check_minimum_cvss) -DassemblyAnalyzerEnabled=$(dependency_check_assembly_analyzer_enabled) -DsuppressionFiles=$(suppressions_file)-DnvdApiKey="18ddeb2f-fa5b-41ac-a6c2-69b1afe075cd"; \
	else \
